...
class Algorithm:
    ...
    def integrate_multiple_steps(self, duration: float, control: torch.Tensor):
        ...
        assert len(control) == len(self.agent.zeta) == 6
        x = self.agent.zeta + self.agent.x_star
        rate = self.agent.c + control
        new_x = x * torch.exp(rate * duration)
        return new_x - self.agent.x_star
    ...
    def new_state(self, a):
        ...
        for a_, circle in action_circle.items():
            ...
            map_action_control[0] = self.actions_controls[1][int(
